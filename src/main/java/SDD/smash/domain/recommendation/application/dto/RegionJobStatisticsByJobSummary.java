 ...
 * @param selectedJobTopCode  사용자가 고른 중분류가 속한 대분류 코드. 고르지 않았으면 {@code null}.
...
public record RegionJobStatisticsByJobSummary(String statisticsMonth,
    ...
    public static RegionJobStatisticsByJobSummary empty(String selectedJobTopCode) {
        return new RegionJobStatisticsByJobSummary(null, selectedJobTopCode, null, List.of());
    }
    ...
    public static RegionJobStatisticsByJobSummary from(List<RegionJobStatisticsView> views,
        ...
        if (views == null || views.isEmpty()) {
            return empty(selectedJobTopCode);
        }
        ...
        List<RegionJobStatisticsByJobItem> items = views.stream()
                ...
                .map(view -> RegionJobStatisticsByJobItem.from(
                        ...
                        view.jobTopCode().equals(selectedJobTopCode),
