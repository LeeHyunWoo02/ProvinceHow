...
/**
 * LOCALDATA(공공데이터포털 인허가 API) 전용 {@link RestTemplate} 빈.
 *
 * <h2>왜 공유 빈을 쓰지 않는가</h2>
 * {@code global/config/RedisConfig} 의 공유 {@code restTemplate}(connect 5s / read 25s)은
 ...
 * 있다. 그래서 이 빈은 {@code defaultCandidate = false} 로 등록한다 — <b>{@code @Qualifier} 로
 * 이름을 지목한 곳(=LocalDataApiAdapter)에만</b> 주입되고, 타입만 보고 주입하는 다른 어댑터의
 * 후보에는 아예 오르지 않는다.
 */
@Configuration
public class LocalDataRestTemplateConfig {
