...
2. **매핑에 없는 코드는 추정하지 않고 제외**하며 `unmappedRegions` 로 계량해 로그에 남긴다.
   주소 문자열(`LOTNO_ADDR`) 파싱은 **아직 구현하지 않았다** — 코드가 있는데 표가 비어 있는 상황을
   주소 추정으로 덮으면 오류가 조용히 묻힌다.
3. **이 파일은 수집 대상 목록이기도 하다.** 여기 없는 자치단체는 호출하지 않는다.
...
작성 시점의 개발 환경에서 data.go.kr 로 나가는 네트워크가 막혀 있어 전수를 내려받지 못했다.
현재 파일에는 문서에서 확인된 **6건**만 들어 있다.
...
### 5.4 미매칭 36건 — 일반구 35 + 세종 **[확인됨]**

인허가 권한이 일반구가 아니라 **시**에 있어 데이터가 시 단위로만 나온다.
...
세종특별자치시는 시군구 목록이 아니라 **시도 본청 코드 `5690000`** 을 쓴다.

> **미결정 사항**: 일반구 35개를 상위 시의 값으로 채울지, 주소 문자열로 재분배할지는 **정책 결정**이다.
> 현재 구현은 매핑에 있는 것만 적재하므로 일반구는 `infra` 행이 없고, 그 지역의 인프라 점수는 결측이 된다.
> 결측 대분류는 `InfraScorePolicy` 에서 0점으로 합산돼 **추천 점수를 부당하게 낮춘다.** 결정이 필요하다.
...
| # | 항목 | 왜 |
| --- | --- | --- |
| 1 | **개방자치단체코드 261행 전수표** | 개발 환경에서 data.go.kr 접속 불가. 매핑 파일이 6건뿐이다 |
...
| 9 | 일반구 35개 처리 정책 | 상위 시 값 복제 vs 주소 재분배 — 기획 결정 필요 |
