...
**[일부 확인 — 시군구 레벨은 2차 출처]** 시군구 5자리(종로구 `11110`)라는 점은 PublicDataReader 문서(`https://wooiljeong.github.io/python/pdr-kosis-ex1/`)의 실제 응답 DataFrame으로 확인했다. **공식 응답을 직접 호출해 확인하지는 못했다**(인증키 미보유). KOSIS 통계표 페이지는 하위 분류값을 지연 로딩해서 시군구 목록이 HTML에 들어있지 않다.

> **키 발급 후 1회 실호출로 최종 확인을 권한다.**
...
| 4 | KOSIS `DT_1B040A3` 의 `C1` 시군구 5자리 실응답 | 인증키 미보유. 시도 2자리까지만 공식 확인. **키 발급 후 1회 호출로 검증 권장** |
