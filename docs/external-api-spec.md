...
| `LAWD_CD` | 공식 명칭 | 202605 AptRent | 관할 법정동 — `umdNm` 실측 |
| --- | --- | --- | --- |
| `41591` | **화성시 만세구** | 708 | 남양읍, 향남읍, 우정읍, 팔탄면, 새솔동 |
| `41593` | **화성시 효행구** | 214 | 봉담읍, 기안동, 비봉면 |
| `41595` | 화성시 병점구 | 404 | 병점동, 진안동, 반월동, 기산동, 안녕동, 송산동, 능동 |
| `41597` | 화성시 동탄구 | 1,037 | 반송동, 석우동, 능동, 목동, 산척동, 신동, 여울동 (동탄) |

> **구 명칭 [확인됨]** — KOSIS 통계 API(`orgId=101` / `tblId=DT_1B040A3`, 기준월 `202607`)의
> 시군구 분류값과 대조했다(2026-08-28). 실거래 API 는 명칭을 주지 않으므로 코드↔명칭의 근거는 KOSIS 다.
>
> **처음에는 관할 법정동으로 명칭을 추론했고 4개 중 2개가 틀렸다** — `41591` 을 `남양구`,
> `41593` 을 `봉담구` 로 적었다(각각 남양읍·봉담읍이 관할이라서). 실제는 `만세구`·`효행구` 다.
> `docs/batch-data-pipeline.md` §6 의 "명칭 유사도로 추론하지 않는다" 원칙이 필요한 이유의 실례다.
...
| 신규 코드 | 공식 명칭 | 202605 | 관할 법정동 — `umdNm` 실측 |
| --- | --- | --- | --- |
| `28125` | 제물포구 | 105 | 답동, 만석동, 송림동, 송현동, 신흥동, 전동, 항동 |
| `28155` | 영종구 | 391 | 운서동, 운남동, 운북동, 중산동 (영종도) |
| `28275` | **서해구** | 473 | 가정동, 가좌동, 검암동, 경서동, 석남동, 신현동, 심곡동, 연희동 |
| `28290` | 검단구 | 467 | 당하동, 마전동, 원당동, 금곡동, 백석동, 불로동, 왕길동 (검단) |
...
`sigungu.csv` 에 등재한 명칭은 `28125` 제물포구 / `28155` 영종구 / `28275` 서해구 / `28290` 검단구 다.

> **근거의 범위를 구분한다.**
> - **[확인됨, 실측]** 신규 4개 코드가 실거래 데이터를 반환한다는 사실과, 각 코드의 **관할 법정동 구성**. `umdNm` 을 직접 확인했다.
> - **[확인됨]** **구 명칭**. KOSIS 통계 API(`orgId=101` / `tblId=DT_1B040A3`, 기준월 `202607`)의 시군구 분류값과 대조했다(2026-08-28). 실거래 API 는 명칭을 주지 않으므로 근거는 KOSIS 다.
>   - **처음에는 관할 법정동으로 추론했고 `28275` 를 `서구` 로 적었다. 실제는 `서해구` 다.** 화성 2건(§3.7)까지 합쳐 **추론한 8개 중 3개가 틀렸다.** `docs/batch-data-pipeline.md` §6 의 "명칭 유사도로 추론하지 않는다" 원칙이 필요한 이유의 실례다.
...
### KOSIS 가 뒷받침하는 것 — 옛 코드 3개의 폐지 **[확인됨, 실측]**

KOSIS(`orgId=101` / `tblId=DT_1B040A3`) **202607** 시점의 시군구 분류값에 `28110`(중구)·`28140`(동구)·`28260`(서구)이 **아예 없다.**
대신 `28114 중구영종출장소` / `28118 중구용유출장소` / `28265 서구검단출장소` 가 있다.
세 코드가 폐지됐다는 방증이다.

> **출장소 코드는 `sigungu.csv` 에 추가하지 않는다.** MOLIT 은 출장소 코드에 데이터를 주지 않는다.
> `28100~28299` 를 전수 탐색한 결과 실거래를 반환하는 코드는
> `28125`·`28155`·`28177`·`28185`·`28200`·`28237`·`28245`·`28275`·`28290` 뿐이다.

> 옛 코드 3개는 CSV에서 제거하지 않았다(적재 Writer 가 merge 라 무효). 따라서 인천 서부는
> **`28260`(인프라 있음·전월세 없음)** 과 **`28275`(전월세 있음·인프라 없음)** 로 갈린 상태가 그대로 남는다.
...
| ~~22~~ | ~~인천 신규 4개 코드(`28125`/`28155`/`28275`/`28290`)의 **구 명칭**~~ | **해소됨(2026-08-28)**. KOSIS `orgId=101`/`tblId=DT_1B040A3` `202607` 대조로 확정. `28275` 는 `서구` 가 아니라 **`서해구`** 였다 (3.8절) |
